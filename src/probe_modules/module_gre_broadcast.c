...
probe_module_t module_gre_broadcast;
...
int gre_broadcast_global_initialize(struct state_conf *conf)
{
	return EXIT_SUCCESS;
...
int gre_broadcast_init_perthread(void *buf, macaddr_t *src,
					...
					UNUSED void **arg_ptr)
{
	...
	uint16_t len = htons(sizeof(struct ip) * 2 +  sizeof(struct ether_header) + sizeof(gre_header_t) + sizeof(struct icmp) + INET_ADDRSTRLEN);
	...
	// Use broadcast MAC destination address and a random source MAC address
...
int gre_broadcast_make_packet(void *buf, size_t *buf_len,
				     ...
				     UNUSED int probe_num, UNUSED void *arg)
{
	...
	// This is set by the command line, it should be set to the glboal IPv4 address in case of AWS (instance where interface IP is not global IP)
	...
	// Inner packet has broadcast destination IP 
    inet_pton(AF_INET,"255.255.255.255",&(ip_header2->ip_dst));
	...
	icmp_header->icmp_cksum = icmp_checksum((unsigned short *)icmp_header, sizeof(struct icmp) + strlen(payload));
	ip_header2->ip_ttl = 2;
	...
	*buf_len = sizeof(struct ether_header)*2 + sizeof(struct ip)*2 + sizeof(gre_header_t )+ sizeof(struct icmp) + INET_ADDRSTRLEN;
	return EXIT_SUCCESS;
}

void gre_broadcast_print_packet(FILE *fp, void *packet)
{
...
void gre_broadcast_process_packet(const u_char *packet, UNUSED uint32_t len,
			 ...
			 UNUSED const struct timespec ts)
{
	...
	// Extract IPv4 address from payload 
	char *payload =
...
static int gre_broadcast_validate_id_seq(struct icmp *icmp_h, const struct ip *ip_hdr)
{
...
static int gre_broadcast_validate_packet(const struct ip *ip_hdr, uint32_t len,
				UNUSED uint32_t *src_ip, uint32_t *validation)
{
	...
	if (icmp_h->icmp_type == ICMP_ECHOREPLY) {
		return gre_broadcast_validate_id_seq(icmp_h, ip_hdr);
	} else {
		...
		return gre_broadcast_validate_id_seq(icmp_inner, ip_hdr);
	}
}
...
probe_module_t module_gre_broadcast = {
    .name = "gre_broadcast",
    .max_packet_length = sizeof(struct ether_header)*2 + sizeof(struct ip) * 2 + sizeof(gre_header_t) +
		     sizeof(struct icmp) + INET_ADDRSTRLEN,
    .pcap_filter = "icmp",
    .pcap_snaplen = 1500,
    .port_args = 0,
    .thread_initialize = &gre_broadcast_init_perthread,
    .global_initialize = &gre_broadcast_global_initialize,
    .make_packet = &gre_broadcast_make_packet,
    .print_packet = &gre_broadcast_print_packet,
    .validate_packet = &gre_broadcast_validate_packet,
    .process_packet = &gre_broadcast_process_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 8};
