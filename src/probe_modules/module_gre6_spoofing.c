...
probe_module_t module_gre6_spoofing;
...
int gre6_spoofing_global_initialize(struct state_conf *conf)
{
	...
	if (asprintf((char **restrict)&module_gre6_spoofing.pcap_filter,
		     "%s && ip6 dst host %s", module_gre6_spoofing.pcap_filter,
...
static int gre6_spoofing_init_perthread(void *buf, macaddr_t *src,
					macaddr_t *gw,
...
static int gre6_spoofing_make_packet(void *buf, size_t *buf_len,
				     ...
				     UNUSED void *arg)
{
	...
	ip6_header2->ip6_src = ((struct in6_addr *)arg)[2]; //Spoofed address
...
static void gre6_spoofing_print_packet(FILE *fp, void *packet)
{
...
static int gre6_spoofing_validate_packet(const struct ip *ip_hdr, uint32_t len,
					 __attribute__((unused))
					 uint32_t *src_ip,
					 uint32_t *validation)
{
...
static void gre6_spoofing_process_packet(const u_char *packet,
...
probe_module_t module_gre6_spoofing = {
    ...
    .port_args = 0,
    .global_initialize = &gre6_spoofing_global_initialize,
    .thread_initialize = &gre6_spoofing_init_perthread,
    .make_packet = &gre6_spoofing_make_packet,
    .print_packet = &gre6_spoofing_print_packet,
    .process_packet = &gre6_spoofing_process_packet,
    .validate_packet = &gre6_spoofing_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 7};
