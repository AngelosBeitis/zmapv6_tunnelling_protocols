...
probe_module_t module_4in6_spoofing;

int fourIn6_spoofing_global_initialize(struct state_conf *conf)
{
	if(!zconf.spoofing_address_v4){
		log_error("4in6_spoofing", "Spoofing address was not given. Add --spoofing-address-v4");
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
...
static int
fourIn6_spoofing_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
				...
				__attribute__((unused)) void **arg_ptr)
{
	...
	uint16_t payload_len =
	    sizeof(struct ip) + sizeof(struct icmp) + INET6_ADDRSTRLEN;
	make_ip6_header(ip6_header, IPPROTO_IPIP, payload_len);

	struct ip *ip_header = (struct ip *)(&ip6_header[1]);
	// ICMPv6 header plus 8 bytes of data (validation)
	payload_len =
	    htons(sizeof(struct ip) + sizeof(struct icmp) + INET6_ADDRSTRLEN);
...
static int fourIn6_spoofing_make_packet(void *buf, size_t *buf_len,
					ipaddr_n_t src_ip,
					...
					UNUSED int probe_num, void *arg)
{
	...
	ip_header->ip_src = *addr4_1; // Spoofed address
	ip_header->ip_dst = *addr4_2; //external address

	if (ip_header->ip_dst.s_addr == INADDR_ANY) {
		ip_header->ip_dst.s_addr = src_ip;
	}
...
static void fourIn6_spoofing_print_packet(FILE *fp, void *packet)
{
...
static int fourIn6_spoofing_validate_packet(const struct ip *ip_hdr,
					    uint32_t len,
...
static void fourIn6_spoofing_process_packet(
...
probe_module_t module_4in6_spoofing = {
    ...
    .port_args = 0,
    .global_initialize = &fourIn6_spoofing_global_initialize,
    .thread_initialize = &fourIn6_spoofing_init_perthread,
    .make_packet = &fourIn6_spoofing_make_packet,
    .print_packet = &fourIn6_spoofing_print_packet,
    .process_packet = &fourIn6_spoofing_process_packet,
    .validate_packet = &fourIn6_spoofing_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 7};