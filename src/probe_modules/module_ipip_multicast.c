...
const char *ipip_multicast_usage_error =
    "unknown UDP probe specification (expected file:/path or text:STRING or hex:01020304)";

probe_module_t module_ipip_multicast;

int ipip_multicast_global_initialize(struct state_conf *conf)
{
	return EXIT_SUCCESS;
}


int ipip_multicast_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
...
int ipip_multicast_make_packet(void *buf, size_t *buf_len, ipaddr_n_t src_ip,
...
void ipip_multicast_print_packet(FILE *fp, void *packet)
{
...
void ipip_multicast_process_packet(const u_char *packet, UNUSED uint32_t len,
...
static int ipip_multicast_validate_id_seq(struct icmp *icmp_h, uint32_t *validation)
{
...
static int ipip_multicast_validate_packet(const struct ip *ip_hdr, uint32_t len,
				UNUSED uint32_t *src_ip, uint32_t *validation)
{
	...
	if (icmp_h->icmp_type == ICMP_ECHOREPLY) {
		return ipip_multicast_validate_id_seq(icmp_h, validation);
	} else {
		...
		return ipip_multicast_validate_id_seq(icmp_inner, validation);
	}
}
...
probe_module_t module_ipip_multicast = {
    ...
    .pcap_snaplen = 1500,
    .port_args = 0,
    .thread_initialize = &ipip_multicast_init_perthread,
    .global_initialize = &ipip_multicast_global_initialize,
    .make_packet = &ipip_multicast_make_packet,
    .print_packet = &ipip_multicast_print_packet,
    .validate_packet = &ipip_multicast_validate_packet,
    .process_packet = &ipip_multicast_process_packet,
    .close = NULL,
