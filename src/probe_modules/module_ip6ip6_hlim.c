...
probe_module_t module_ip6ip6_hlim;

int ip6ip6_hlim_global_initialize(struct state_conf *conf)
{
	// Only look at received packets destined to the specified scanning address (useful for parallel zmap scans)
	if (asprintf((char **restrict)&module_ip6ip6_hlim.pcap_filter,
		     "%s && ip6 dst host %s", module_ip6ip6_hlim.pcap_filter,
...
static int ip6ip6_hlim_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
...
static int ip6ip6_hlim_make_packet(void *buf, size_t *buf_len,
...
static void ip6ip6_hlim_print_packet(FILE *fp, void *packet)
{
...
static int ip6ip6_hlim_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
static void ip6ip6_hlim_process_packet(const u_char *packet,
...
probe_module_t module_ip6ip6_hlim = {
    ...
    .port_args = 0,
    .global_initialize = &ip6ip6_hlim_global_initialize,
    .thread_initialize = &ip6ip6_hlim_init_perthread,
    .make_packet = &ip6ip6_hlim_make_packet,
    .print_packet = &ip6ip6_hlim_print_packet,
    .process_packet = &ip6ip6_hlim_process_packet,
    .validate_packet = &ip6ip6_hlim_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 7};
