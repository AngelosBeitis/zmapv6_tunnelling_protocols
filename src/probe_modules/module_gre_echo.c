...
probe_module_t module_gre_echo;
...
int gre_echo_global_initialize(struct state_conf *conf)
{
	return EXIT_SUCCESS;

}

int gre_echo_init_perthread(void *buf, macaddr_t *src,
...
int gre_echo_make_packet(void *buf, size_t *buf_len,
...
void gre_echo_print_packet(FILE *fp, void *packet)
{
...
void gre_echo_process_packet(const u_char *packet, UNUSED uint32_t len,
...
static int gre_echo_validate_id_seq(struct icmp *icmp_h, uint32_t *validation)
{
...
static int gre_echo_validate_packet(const struct ip *ip_hdr, uint32_t len,
				UNUSED uint32_t *src_ip, uint32_t *validation)
{
	...
	if (icmp_h->icmp_type == ICMP_ECHOREPLY) {
		return gre_echo_validate_id_seq(icmp_h, validation);
	} else {
		...
		return gre_echo_validate_id_seq(icmp_inner, validation);
	}
}
...
probe_module_t module_gre_echo = {
    ...
    .pcap_snaplen = 1500,
    .port_args = 0,
    .thread_initialize = &gre_echo_init_perthread,
    .global_initialize = &gre_echo_global_initialize,
    .make_packet = &gre_echo_make_packet,
    .print_packet = &gre_echo_print_packet,
    .validate_packet = &gre_echo_validate_packet,
    .process_packet = &gre_echo_process_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 7};
