...
probe_module_t module_6in4_ttl;

int sixIn4_ttl_global_initialize(struct state_conf *conf)
{
	if(!zconf.external_address_v6){
		log_error("6in4_ttl", "External IPv6 address was not given. Add --external-ipv6-address");
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}

static int sixIn4_ttl_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
				     ...
				     UNUSED void **arg_ptr)
{
	...
	uint16_t payload_len = htons(
	    ...
	    sizeof(struct icmp6_hdr) + INET_ADDRSTRLEN);
...
static int sixIn4_ttl_make_packet(void *buf, size_t *buf_len, ipaddr_n_t src_ip,
				  ...
				  UNUSED void *arg)
{
	...
	ip6_header->ip6_src = ((struct in6_addr *)arg)[0]; // External IPv6 address


	four_to_six(dst_ip, &ip6_header->ip6_dst);
	...
	*buf_len = sizeof(struct ether_header) + sizeof(struct ip) +
		   sizeof(struct ip6_hdr) + sizeof(struct icmp6_hdr) + INET_ADDRSTRLEN;

	return EXIT_SUCCESS;
}

static void sixIn4_ttl_print_packet(FILE *fp, void *packet)
{
...
static int sixIn4_ttl_validate_packet(const struct ip *ip_hdr, uint32_t len,
				      ...
				      uint32_t *validation)
{
	...
	struct ip6_hdr *ip6_header2 = (struct ip6_hdr *)(&icmp6_h[1]);
	
	if (ip6_header2->ip6_ctlun.ip6_un1.ip6_un1_nxt != IPPROTO_ICMPV6) {
		return PACKET_INVALID;
	}
...
static void sixIn4_ttl_process_packet(const u_char *packet,
				      ...
				      uint32_t *validation)
{
	...
	if (icmp6_hdr->icmp6_type == ICMP6_TIME_EXCEEDED) {
		...
		case ICMP6_ECHO_REPLY:
			fs_add_string(fs, "classification", (char *)"echoreply",
				      0);
			break;
		default:
...
probe_module_t module_6in4_ttl = {
    ...
    .port_args = 0,
    .global_initialize = &sixIn4_ttl_global_initialize,
    .thread_initialize = &sixIn4_ttl_init_perthread,
    .make_packet = &sixIn4_ttl_make_packet,
    .print_packet = &sixIn4_ttl_print_packet,
    .process_packet = &sixIn4_ttl_process_packet,
    .validate_packet = &sixIn4_ttl_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 8};