...
int gre_nat_global_initialize(struct state_conf *conf)
{
	if(!zconf.spoofing_address_v4){
		log_error("gre_nat", "Spoofing IPv4 address was not given. Add --spoofing-address-v4");
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}

static int gre_nat_init_perthread(void *buf, macaddr_t *src,
					...
					UNUSED void **arg_ptr)
{
	...
	uint16_t payload_len = htons(sizeof(struct ip) + sizeof(gre_header_t) +sizeof(struct ip) +
...
static int gre_nat_make_packet(void *buf, size_t *buf_len,
...
static void gre_nat_print_packet(FILE *fp, void *packet)
{
...
static int gre_nat_validate_id_seq(struct icmp *icmp_h, const struct ip *ip_hdr)
{
...
static int gre_nat_validate_packet(const struct ip *ip_hdr, uint32_t len,
					 ...
					 uint32_t *validation)
{
	...
	return gre_nat_validate_id_seq(icmp_h,ip_hdr);
}

static void gre_nat_process_packet(const u_char *packet,
...
probe_module_t module_gre_nat = {
    ...
    .port_args = 0,
    .global_initialize = &gre_nat_global_initialize,
    .thread_initialize = &gre_nat_init_perthread,
    .make_packet = &gre_nat_make_packet,
    .print_packet = &gre_nat_print_packet,
    .process_packet = &gre_nat_process_packet,
    .validate_packet = &gre_nat_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 7};