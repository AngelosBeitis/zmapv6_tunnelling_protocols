...
probe_module_t module_ipip_doubleproxy;

int ipip_doubleproxy_global_initialize(struct state_conf *conf)
{
	return EXIT_SUCCESS;
}


int ipip_doubleproxy_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
...
int ipip_doubleproxy_make_packet(void *buf, size_t *buf_len, ipaddr_n_t src_ip,
...
void ipip_doubleproxy_print_packet(FILE *fp, void *packet)
{
...
void ipip_doubleproxy_process_packet(const u_char *packet, UNUSED uint32_t len,
...
static int ipip_doubleproxy_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
probe_module_t module_ipip_doubleproxy = {
    .name = "ipip_doubleproxy",
    .max_packet_length = sizeof(struct ether_header) + sizeof(struct ip) * 2 +
		     sizeof(struct icmp),
    .pcap_filter = "icmp[0]==8",
    .pcap_snaplen = 1500,
    .port_args = 0,
    .thread_initialize = &ipip_doubleproxy_init_perthread,
    .global_initialize = &ipip_doubleproxy_global_initialize,
    .make_packet = &ipip_doubleproxy_make_packet,
    .print_packet = &ipip_doubleproxy_print_packet,
    .validate_packet = &ipip_doubleproxy_validate_packet,
    .process_packet = &ipip_doubleproxy_process_packet,
    .close = NULL,
