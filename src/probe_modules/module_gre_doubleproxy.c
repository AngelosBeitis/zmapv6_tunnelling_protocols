...
probe_module_t module_gre_doubleproxy;
...
int gre_doubleproxy_global_initialize(struct state_conf *conf)
{
	return EXIT_SUCCESS;

}

int gre_doubleproxy_init_perthread(void *buf, macaddr_t *src,
					...
					UNUSED void **arg_ptr)
{
	...
	uint16_t len = htons(sizeof(struct ip) * 2 + sizeof(gre_header_t) + sizeof(struct icmp));
...
int gre_doubleproxy_make_packet(void *buf, size_t *buf_len,
				     ...
				     UNUSED int probe_num, UNUSED void *arg)
{
	...
	ip_header2->ip_src = ((struct in_addr *)arg)[0];// Spoofed IPv4
	if (ip_header2->ip_src.s_addr == INADDR_ANY) {
		ip_header2->ip_src.s_addr = src_ip;
	}
	ip_header2->ip_dst = ((struct in_addr *)arg)[1];// External IPv4

	uint16_t icmp_idnum = (dst_ip >> 16) & 0xFFFF;
	uint16_t icmp_seqnum = dst_ip & 0xFFFF;
	...
	*buf_len = sizeof(struct ether_header) + sizeof(struct ip) *2 + sizeof(gre_header_t )+ sizeof(struct icmp);
	return EXIT_SUCCESS;
}

void gre_doubleproxy_print_packet(FILE *fp, void *packet)
{
...
void gre_doubleproxy_process_packet(const u_char *packet, UNUSED uint32_t len,
...
static int gre_doubleproxy_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
probe_module_t module_gre_doubleproxy = {
    .name = "gre_doubleproxy",
    .max_packet_length = sizeof(struct ether_header) + sizeof(struct ip) * 2 + sizeof(gre_header_default) +
		     sizeof(struct icmp),
    .pcap_filter =  "icmp[0]==8",
    .pcap_snaplen = 1500,
    .port_args = 0,
    .thread_initialize = &gre_doubleproxy_init_perthread,
    .global_initialize = &gre_doubleproxy_global_initialize,
    .make_packet = &gre_doubleproxy_make_packet,
    .print_packet = &gre_doubleproxy_print_packet,
    .validate_packet = &gre_doubleproxy_validate_packet,
    .process_packet = &gre_doubleproxy_process_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 7};
