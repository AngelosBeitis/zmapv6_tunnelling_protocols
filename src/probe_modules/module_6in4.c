...
probe_module_t module_6in4;

int sixIn4_global_initialize(struct state_conf *conf)
{
	if(!zconf.external_address_v6 || !zconf.spoofing_address_v6){
		log_error("6in4", "External IPv6 address and/or spoofing address were not given. Add --external-ipv6-address and --spoofing-address-v6");
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}

static int sixIn4_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
...
static int sixIn4_make_packet(void *buf, size_t *buf_len,
				   ...
				   UNUSED int probe_num, void *arg)
{
	...
	ip6_header->ip6_src = ((struct in6_addr *)arg)[0]; //spoofed IPv6 address
	ip6_header->ip6_dst = ((struct in6_addr *)arg)[1]; // External IPv6 address
...
static void sixIn4_print_packet(FILE *fp, void *packet)
{
...
static int sixIn4_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
static void sixIn4_process_packet(const u_char *packet,
				       ...
				       uint32_t *validation)
{
	...
	char *payload =(char *)&packet[sizeof(struct ether_header) + sizeof(struct ip6_hdr) + sizeof(struct icmp6_hdr)];
...
probe_module_t module_6in4 = {
    ...
    .port_args = 0,
    .global_initialize = &sixIn4_global_initialize,
    .thread_initialize = &sixIn4_init_perthread,
    .make_packet = &sixIn4_make_packet,
    .print_packet = &sixIn4_print_packet,
    .process_packet = &sixIn4_process_packet,
    .validate_packet = &sixIn4_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 8};